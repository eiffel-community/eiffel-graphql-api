...
class Environment(graphene.ObjectType):
    ...
    def resolve_environment_defined(self, info):
        from ..union import NotFound
        event = find_one("EiffelEnvironmentDefinedEvent", {"meta.id": self.link.get("target")})
        if event is None:
            return NotFound(self.link, "Could not find event in database.")
        return EnvironmentDefined(event)


class EnvironmentPreviousVersion(Environment): pass
