...
class ActivityExecution(graphene.ObjectType):
    activity_triggered = graphene.Field(ActivityTriggered)

    def __init__(self, link):
        self.link = link

    def resolve_activity_triggered(self, info):
        from ..union import NotFound
        event = find_one("EiffelActivityTriggeredEvent", {"meta.id": self.link.get("target")})
        if event is None:
            return NotFound(self.link, "Could not find event in database.")
        return ActivityTriggered(event)


class PreviousActivityExecution(ActivityExecution):
    pass
